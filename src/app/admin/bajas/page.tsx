...
const TIPO_LABEL: Record<string,string> = { enfermedad_comun:'Enfermedad comun', accidente_laboral:'Accidente laboral', maternidad_paternidad:'Maternidad/Paternidad', accidente_no_laboral:'Accidente no laboral', cuidado_familiar:'Cuidado familiar' }
...
export default function BajasAdminPage() {
  ...
  const filtradas = useMemo(() => {
    let list = bajas
    if (busqueda) { const q=busqueda.toLowerCase(); list=list.filter(b=>(b.empleados as any).nombre.toLowerCase().includes(q)||(TIPO_LABEL[b.tipo]||b.tipo).toLowerCase().includes(q)) }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="page-header">
        <div><h1 className="page-title">Bajas</h1><p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{bajas.filter(b=>b.activa).length} activas de {bajas.length}</p></div>
      </div>
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="relative"><Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400"/><input value={busqueda} onChange={e=>setBusqueda(e.target.value)} className="input pl-9 w-52" placeholder="Buscar…"/></div>
        <div className="flex rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          {(['todas','activa','finalizada'] as const).map(f=>(
            <button key={f} onClick={()=>setFiltroActiva(f)} className={`px-4 py-2 text-xs font-medium capitalize transition-colors ${filtroActiva===f?'bg-indigo-600 text-white':'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>{f}</button>
          ))}
        </div>
      ...
      {loading ? <SkeletonTable rows={4}/> : filtradas.length===0 ? (
        <EmptyState icon="checkmark" title="Sin bajas" description={busqueda?'Sin resultados para tu busqueda':'No hay bajas registradas'}/>
      ) : (
              ...
              {filtradas.map(b=>(
                <tr key={b.id} className="table-row">
                  <td className="table-cell"><div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{backgroundColor:(b.empleados as any).avatar_color||'#6366f1'}}>{(b.empleados as any).nombre.split(' ').map((n:string)=>n[0]).join('').substring(0,2)}</div>
