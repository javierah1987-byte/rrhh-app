...
function SortIcon({ active, dir }: { active:boolean; dir:SortDir }) {
  ...
  return dir==='asc' ? <ChevronUp className="w-3.5 h-3.5 text-indigo-600"/> : <ChevronDown className="w-3.5 h-3.5 text-indigo-600"/>
}
...
export default function EmpleadosPage() {
  ...
  const filtrados = useMemo(() => {
    ...
    if (busqueda) {
      ...
      list = list.filter(e => e.nombre?.toLowerCase().includes(q) || e.departamento?.toLowerCase().includes(q) || e.puesto?.toLowerCase().includes(q) || e.email?.toLowerCase().includes(q))
    }
    return [...list].sort((a,b) => {
      const cmp = String(a[sortKey]||'').localeCompare(String(b[sortKey]||''),'es',{numeric:true,sensitivity:'base'})
      return sortDir==='asc'?cmp:-cmp
    })
  }, [empleados, busqueda, sortKey, sortDir])

  const cols: {key:SortKey;label:string}[] = [
    {key:'nombre',label:'Empleado'},{key:'departamento',label:'Departamento'},
    {key:'puesto',label:'Puesto'},{key:'tipo_contrato',label:'Contrato'},
    {key:'fecha_alta',label:'Alta'},{key:'estado',label:'Estado'},
  ]
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{empleados.filter(e=>e.estado==='activo').length} activos de {empleados.length}</p>
        </div>
      ...
      {loading ? <SkeletonTable rows={5}/> : filtrados.length===0 ? (
        ...
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead><tr>
              {cols.map(col=>(
                <th key={col.key} className="table-header cursor-pointer select-none group" onClick={()=>handleSort(col.key)}>
                  <div className="flex items-center gap-1">{col.label}<SortIcon active={sortKey===col.key} dir={sortDir}/></div>
                </th>
              ))}
              ...
              {filtrados.map(e=>(
                <tr key={e.id} className="table-row cursor-pointer" onClick={()=>router.push(`/admin/empleados/${e.id}`)}>
                  ...
                  <td className="table-cell" onClick={ev=>ev.stopPropagation()}>
                    <select value={e.estado} onChange={async ev=>{ev.stopPropagation();await supabase.from('empleados').update({estado:ev.target.value}).eq('id',e.id);setEmpleados(prev=>prev.map(x=>x.id===e.id?{...x,estado:ev.target.value}:x))}}
                  ...
                  <td className="table-cell" onClick={ev=>ev.stopPropagation()}>
                    <button onClick={()=>router.push(`/admin/empleados/${e.id}`)} className="flex items-center gap-1 text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium">
          ...
          <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-700 text-xs text-slate-400">{filtrados.length} empleado{filtrados.length!==1?'s':''}{busqueda&&` de ${empleados.length}`}</div>
        </div>
      )}
    </div>
  )
