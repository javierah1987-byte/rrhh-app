...
const CATS:Record<string,string>={transporte:'🚗',comida:'🍽️',alojamiento:'🏨',material:'📦',telefono:'📱',formacion:'📚',otro:'📎'}
...
export default function AdminGastosPage(){
  ...
  async function gestionar(id:string,estado:'aprobado'|'rechazado'){
    ...
    await supabase.from('gastos').update({estado,comentario_admin:com||null}).eq('id',id)
    ...
    setGastos(p=>p.map(x=>x.id===id?{...x,estado,comentario_admin:com||null}:x))
    setProcesando(null)
  }
  ...
  return(
    <div>
      <div className="page-header mb-5"><div><h1 className="page-title">Gastos del equipo</h1><p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Revisa y aprueba los gastos profesionales</p></div><div className="card px-4 py-2 text-right"><p className="text-xs text-slate-400">Pendiente aprobación</p><p className="text-xl font-black text-amber-600">{totalPend.toFixed(2)} €</p></div></div>
      <div className="flex flex-wrap gap-2 mb-5">
        {[{k:'pendiente',l:'Pendientes',c:counts.pendiente,a:'bg-amber-500 text-white',i:'bg-amber-50 text-amber-700'},{k:'aprobado',l:'Aprobados',c:counts.aprobado,a:'bg-emerald-500 text-white',i:'bg-emerald-50 text-emerald-700'},{k:'rechazado',l:'Rechazados',c:counts.rechazado,a:'bg-red-500 text-white',i:'bg-red-50 text-red-700'},{k:'todas',l:'Todos',c:gastos.length,a:'bg-indigo-600 text-white',i:'bg-slate-100 text-slate-700'}].map(f=>(
          <button key={f.k} onClick={()=>setFiltro(f.k)} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-medium text-sm transition-colors ${filtro===f.k?f.a:f.i}`}>
            {f.l}<span className={`text-xs px-1.5 py-0.5 rounded-full ${filtro===f.k?'bg-white/20':'bg-white/60'}`}>{f.c}</span>
          </button>
        ))}
      </div>
      ...
      :<div className="space-y-3">{filtradas.map(g=>{const emp=g.empleados as any;return(
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span>{CATS[g.categoria]||'📎'}</span>
                  <span className={`badge text-xs capitalize ${g.estado==='pendiente'?'badge-amber':g.estado==='aprobado'?'badge-green':'badge-red'}`}>{g.estado}</span>
                </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              {g.estado==='pendiente'&&(
                <div className="flex flex-col gap-1.5">
                  <input type="text" placeholder="Nota (opcional)" value={comentarios[g.id]||''} onChange={e=>setComentarios(p=>({...p,[g.id]:e.target.value}))} className="input text-xs py-1 w-36"/>
                  <div className="flex gap-1.5">
                    <button onClick={()=>gestionar(g.id,'rechazado')} disabled={procesando===g.id} className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 font-semibold text-xs disabled:opacity-50"><XCircle className="w-3 h-3"/>Rechazar</button>
                    <button onClick={()=>gestionar(g.id,'aprobado')} disabled={procesando===g.id} className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 font-semibold text-xs disabled:opacity-50"><CheckCircle className="w-3 h-3"/>Aprobar</button>
                  </div>
                </div>
              )}
            </div>
