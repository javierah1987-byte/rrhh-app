...
import { Shield, LogOut, RefreshCw, Plus, X, Trash2, AlertTriangle, PauseCircle,
  PlayCircle, CheckCircle, Users, TrendingUp, Layers, Search } from 'lucide-react'
import EmpCard from './EmpCard'

const PC = { starter:'#6366f1', professional:'#10b981', enterprise:'#f59e0b', fichaje:'#0891b2' }
const PL = { starter:'Starter', professional:'Professional', enterprise:'Enterprise', fichaje:'Solo Fichaje' }

export default function SuperAdminMain({ empresas, grupos, planes, features, planFeats,
  configs, overrides, loading, cargar, onLogout }) {
  ...
  const confirmarSusp = async () => {
    ...
    const nuevo = !confirmSusp.suspendida
    const payload = { suspendida: nuevo, motivo_suspension: nuevo ? (motivo || 'Impago') : null, fecha_suspension: nuevo ? new Date().toISOString() : null }
    if (confirmSusp.isGrupo) {
      await supabase.from('grupos').update(payload).eq('id', confirmSusp.id)
      await supabase.from('empresas').update({ suspendida: nuevo }).eq('grupo_id', confirmSusp.id)
    } else {
  ...
  const empBuscar = search ? empresas.filter(e => e.nombre.toLowerCase().includes(search.toLowerCase()) || (e.email || '').toLowerCase().includes(search.toLowerCase())) : []

  const tabClass = (id) => 'px-4 py-3.5 text-sm font-semibold border-b-2 whitespace-nowrap transition-all ' + (tab === id ? 'border-indigo-600 text-indigo-700 bg-indigo-50/50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50')
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="bg-white border-b border-slate-200 px-4 sm:px-6 flex gap-0 overflow-x-auto">
        <button onClick={() => setTab('clientes')} className={tabClass('clientes')}>🏢 Clientes</button>
        <button onClick={() => setTab('grupos')}   className={tabClass('grupos')}>🗂️ Grupos</button>
        <button onClick={() => setTab('planes')}   className={tabClass('planes')}>📊 Planes</button>
        <button onClick={() => setTab('revenue')}  className={tabClass('revenue')}>💰 Ingresos</button>
      </div>
        ...
        {tab === 'clientes' && (
          <div className="space-y-5">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button onClick={() => setModalEmp(true)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold px-4 py-2.5 rounded-xl shadow-sm">
                <Plus className="w-4 h-4"/> Nueva empresa
              </button>
              <button onClick={() => setModalGrupo(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold px-4 py-2.5 rounded-xl shadow-sm">
                <Layers className="w-4 h-4"/> Nuevo grupo
              </button>
            </div>
            {loading ? (
              <div className="text-slate-400 animate-pulse text-center py-10">Cargando...</div>
            ) : search ? (
              <div className="space-y-2">
                {empBuscar.map(emp => <EmpCard key={emp.id} emp={emp} {...cardProps}/>)}
              ...
              <div className="space-y-4">
                {grupos.map(g => {
                  ...
                  const gMrr = gEmps.reduce((s, e) => { const planId = getPlanId(e.id); const plan = planes.find(p => p.id === planId); return s + (plan && !e.suspendida ? +plan.precio_mes * (e.max_empleados || 10) : 0) }, 0)
                  const gTotal = gEmps.reduce((s, e) => s + (e.max_empleados || 0), 0)
                  const wrapCls = g.suspendida ? 'rounded-2xl border-2 border-red-300 overflow-hidden shadow-sm' : 'rounded-2xl border-2 border-purple-200 overflow-hidden shadow-sm'
                  const hdCls = g.suspendida ? 'px-5 py-4 flex items-center gap-3 flex-wrap bg-red-50' : 'px-5 py-4 flex items-center gap-3 flex-wrap bg-purple-50'
                  const avCls = g.suspendida ? 'w-10 h-10 rounded-xl flex items-center justify-center font-black text-white text-lg flex-shrink-0 bg-red-500' : 'w-10 h-10 rounded-xl flex items-center justify-center font-black text-white text-lg flex-shrink-0 bg-purple-600'
                  const sBtnCls = g.suspendida ? 'flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200'
                  return (
                    <div key={g.id} className={wrapCls}>
                      <div className={hdCls}>
                        <div className={avCls}>{g.suspendida ? '⏸' : g.nombre.charAt(0)}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {g.suspendida && <span className="text-[10px] bg-red-100 text-red-700 border border-red-300 px-2 py-0.5 rounded-full font-bold">⏸ SUSP</span>}
                          </div>
                        ...
                        <div className="flex items-center gap-1">
                          <button onClick={() => suspender(g.id, g.nombre, g.suspendida, true)} className={sBtnCls}>
                            {g.suspendida ? <><PlayCircle className="w-3.5 h-3.5"/>Reactivar</> : <><PauseCircle className="w-3.5 h-3.5"/>Suspender</>}
                          </button>
                          <button onClick={() => borrar(g.id, g.nombre, true)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                      ...
                      <div className="px-4 pb-4 pt-3 bg-white space-y-2">
                        ...
                        <button onClick={() => { setFormEmp(f => ({ ...f, grupo_id: g.id })); setModalEmp(true) }}
                          className="w-full flex items-center justify-center gap-2 py-2 text-xs text-purple-600 hover:text-purple-800 border border-dashed border-purple-300 hover:border-purple-500 rounded-xl">
                  ...
                  <div key={g.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="px-5 py-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      {gEmps.map(e => {
                        ...
                        const plan = planes.find(p => p.id === planId)
                        const clr = e.suspendida ? '#ef4444' : (plan ? PC[plan.id] || '#64748b' : '#64748b')
                        const rev = plan && !e.suspendida ? +plan.precio_mes * (e.max_empleados || 10) : 0
                        const rowCls = e.suspendida ? 'flex items-center gap-2.5 p-3 rounded-xl border border-red-200 bg-red-50' : 'flex items-center gap-2.5 p-3 rounded-xl border border-slate-100 bg-slate-50'
                        const revCls = e.suspendida ? 'text-xs font-bold flex-shrink-0 text-red-300 line-through' : 'text-xs font-bold flex-shrink-0 text-emerald-600'
                        return (
                          <div key={e.id} className={rowCls}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-400">{PL[planId] || planId} · {e.max_empleados || 0}u</p>
                            </div>
                            <p className={revCls}>{rev.toLocaleString('es-ES', { minimumFractionDigits: 0 })}€</p>
                          </div>
                        )
                      })}
                    </div>
              ...
              <table className="w-full bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    {planes.map(p => (
                      <th key={p.id} className="px-4 py-3 text-center" style={{ color: PC[p.id] || '#64748b' }}>
                ...
                <tbody>
                  {['tiempo','ausencias','equipo','reclutamiento','admin','comunicacion','cumplimiento','premium','objetivos','tareas','formacion'].map(cat => {
                    ...
                    const lbl = { tiempo:'⏰ Tiempo', ausencias:'📅 Ausencias', equipo:'👥 Equipo', reclutamiento:'💼 Reclutamiento', admin:'⚙️ Admin', comunicacion:'💬 Comunicación', cumplimiento:'🔒 Cumplimiento', premium:'💎 Premium', objetivos:'🎯 Objetivos', tareas:'✅ Tareas', formacion:'📚 Formación' }[cat] || cat
                    return [
                      <tr key={'h-' + cat} className="bg-slate-50/80">
                        <td colSpan={planes.length + 1} className="px-5 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider">{lbl}</td>
                      </tr>,
                      ...feats.map(f => (
                        <tr key={f.id} className="border-t border-slate-50 hover:bg-slate-50/40">
                          ...
                          {planes.map(p => {
                            ...
                            return (<td key={p.id} className="px-4 py-2 text-center">{has ? <CheckCircle className="w-4 h-4 text-emerald-500 mx-auto"/> : <span className="text-slate-200">—</span>}</td>)
                          })}
                        </tr>
                      ))
                    ]
                    ...
                    ...gEmps.map(e => {
                      ...
                      return (
                        <div key={e.id} className="flex items-center gap-3 px-5 py-2 pl-10">
                          <div className="w-6 h-6 rounded-md flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: e.suspendida ? '#ef4444' : (plan ? PC[plan.id] || '#64748b' : '#64748b') }}>{e.nombre.charAt(0)}</div>
                          <div className="flex-1"><p className="text-sm text-slate-600 truncate">{e.nombre}</p><p className="text-xs text-slate-400">{PL[planId] || planId} · {e.max_empleados || 0}u</p></div>
                ...
                {empSolas.map(e => {
                  ...
                  return (
                    <div key={e.id} className="flex items-center gap-3 px-5 py-2.5">
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: e.suspendida ? '#ef4444' : (plan ? PC[plan.id] || '#64748b' : '#64748b') }}>{e.nombre.charAt(0)}</div>
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium text-slate-700 truncate">{e.nombre}</p><p className="text-xs text-slate-400">{PL[planId] || planId} · {e.max_empleados || 0}u</p></div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={formEmp.plan} onChange={e => setFormEmp(p => ({ ...p, plan: e.target.value }))} className="w-full bg-slate-50 rounded-xl px-3 py-2.5 text-sm border border-slate-200 outline-none focus:border-indigo-400">
                    {planes.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Usuarios</label>
                  <input type="number" value={formEmp.max_empleados} onChange={e => setFormEmp(p => ({ ...p, max_empleados: e.target.value }))} className="w-full bg-slate-50 rounded-xl px-3 py-2.5 text-sm border border-slate-200 outline-none focus:border-indigo-400"/>
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Grupo (opcional)</label>
                <select value={formEmp.grupo_id} onChange={e => setFormEmp(p => ({ ...p, grupo_id: e.target.value }))} className="w-full bg-slate-50 rounded-xl px-3 py-2.5 text-sm border border-slate-200 outline-none focus:border-indigo-400">
            ...
            <div className="flex gap-3 px-6 pb-6">
              ...
              <button onClick={crearEmpresa} disabled={savingNew} className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl text-sm font-bold">
      ...
      {modalGrupo && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              {[{ k: 'nombre', l: 'Nombre del grupo *', ph: 'Grupo Axen SL' }, { k: 'email_contacto', l: 'Email', ph: 'admin@grupo.com' }, { k: 'telefono', l: 'Teléfono', ph: '+34 600 000 000' }].map(f => (
                <div key={f.k}>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">{f.l}</label>
            ...
            <div className="flex gap-3 px-6 pb-6">
              ...
              <button onClick={crearGrupo} disabled={savingNew} className="flex-1 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-xl text-sm font-bold">
      ...
      {confirmSusp && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="bg-slate-100 rounded-xl px-4 py-3 mb-4"><p className="font-bold text-slate-800">{confirmSusp.nombre}</p></div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={confirmarSusp} className={'flex-1 py-2.5 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 ' + (confirmSusp.suspendida ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-amber-500 hover:bg-amber-600')}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="bg-red-600 px-6 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                {confirmDel.isGrupo && <p className="text-white/70 text-xs">Las empresas quedan sin grupo (no se borran)</p>}
              </div>
            </div>
            <div className="p-6">
