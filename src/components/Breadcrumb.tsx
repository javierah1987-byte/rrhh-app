...
const ROUTE_LABELS: Record<string, string> = {
  ...
  empleados: 'Empleados', horarios: 'Horarios y turnos',
  'control-horas': 'Control de horas', evaluaciones: 'Evaluaciones',
  vacaciones: 'Solicitudes', bajas: 'Bajas',
  'solicitudes-documentos': 'Peticion docs',
  nominas: 'Nominas', documentos: 'Documentos', informes: 'Informes',
...
export function Breadcrumb({ employeeName }: { employeeName?: string }) {
  ...
  const crumbs = segments.map((seg, i) => ({
    ...
    label: /^[0-9a-f]{8}-/.test(seg) ? (employeeName || 'Ficha') : (ROUTE_LABELS[seg] || seg),
  }))
  return (
      ...
      {crumbs.slice(1).map((crumb, i) => (
        <span key={crumb.href} className="flex items-center gap-1.5">
          ...
          {i === crumbs.length - 2
            ...
            : <button onClick={() => router.push(crumb.href)} className="hover:text-indigo-600 transition-colors">{crumb.label}</button>
          }
        </span>
      ))}
    </nav>
