...
import { Search, Users, Calendar, FileText, Clock, BarChart2, Bell, Home, ArrowRight, DollarSign } from 'lucide-react'
...
const NAV_ITEMS: Result[] = [
  ...
  {label:'Nóminas',href:'/admin/nominas',icon:<DollarSign className="w-4 h-4"/>,type:'nav'},
...
export function CommandPalette(){
  ...
  useEffect(()=>{
    const toggle=()=>{setOpen(o=>!o);setQuery('');setSel(0)}
    const kh=(e:KeyboardEvent)=>{if((e.metaKey||e.ctrlKey)&&e.key==='k'){e.preventDefault();toggle()}; if(e.key==='Escape') setOpen(false)}
    window.addEventListener('keydown',kh)
    window.addEventListener('nexohr-cmdk',toggle as any)
    return()=>{window.removeEventListener('keydown',kh);window.removeEventListener('nexohr-cmdk',toggle as any)}
  },[])
  ...
  const filtered=query.trim()?[...emps,...NAV_ITEMS.filter(i=>i.label.toLowerCase().includes(query.toLowerCase()))]:NAV_ITEMS
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4" onClick={()=>setOpen(false)}>
      ...
      <div className="relative w-full max-w-lg card shadow-2xl animate-scale-in overflow-hidden" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-700">
          ...
          <input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar página o empleado..." className="flex-1 bg-transparent text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 outline-none"/>
        ...
        <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700 flex items-center gap-4">
          {[['↑↓','Navegar'],['↵','Abrir'],['⌘K','Cerrar']].map(([k,l])=>(
            <span key={k} className="text-[10px] text-slate-400 flex items-center gap-1"><kbd className="bg-slate-100 dark:bg-slate-700 px-1 rounded font-mono">{k}</kbd>{l}</span>
          ))}
        </div>
      </div>
    </div>
